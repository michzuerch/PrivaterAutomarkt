<template>
  <div>
    ...
    <div v-show="currentPlace" class="flex flex-col lg:flex-row">
      <div
        class="google-map h-40vh sm:h-65vh flex-grow mx-auto m-4 w-full bg-gray-400 shadow shadow-lg"
        :id="mapName"
      ></div>
      <div
        v-if="parkResultsFound"
        class="w-full lg:w-2/5 lg:m-4 lg:pl-3 lg:pr-6 lg:h-65vh lg:overflow-y-auto"
      >
        <park-result v-for="park in parks" v-bind:key="park.id" :park="park" />
      </div>
    </div>
    <div
      v-show="currentPlace && !parkResultsFound"
      class="mx-auto w-full lg:w-4/5 lg:max-w-5xl"
    >
      <span class="mx-auto text-xl md:text-2xl font-light text-teal-800">
        Sorry, no parks found in this area. Possible issues are searched area is
        too large (like a zip code) or there simply are no parks in that area.
        Please try another search.
      </span>
    </div>
    <div
      v-show="!currentPlace"
      class="my-5 mx-auto w-full lg:w-4/5 lg:max-w-5xl"
    >
      <g-image
        src="~/assets/images/custom.svg"
        class="mx-auto m-5 w-48 sm:w-64"
      />
      <span class="block text-xl md:text-3xl font-light text-teal-800">
        Welcome! Please search for an address or city to find great parks
        nearby!
      </span>
    </div>
  </div>
  ...
  async mounted() {
    try {
      ...
      this.map = new google.maps.Map(
        this.$el.getElementsByClassName("google-map")[0]
      );
